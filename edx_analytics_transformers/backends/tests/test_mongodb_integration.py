...
from edx_analytics_transformers.backends.mongodb import MongoBackend
from edx_analytics_transformers.backends.tests import InMemoryBackend, IntegrationTestCase
from edx_analytics_transformers.tracker import Tracker
...
class TestMongoIntegration(IntegrationTestCase):
    ...
    def setUp(self):
        ...
        self.database_name = 'test_edx_analytics_transformers_' + str(uuid4())
    ...
    def test_sequential_events(self):
        ...
        for i in range(10):
            self.tracker.emit('org.test.user.login', {
                'username': 'tester',
                'user_id': 10,
                'email': 'tester@edx_analytics_transformers.org',
                'sequence': i,
                'current_time': now
            })
